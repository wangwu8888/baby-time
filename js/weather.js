...
renderInputBar:function(){
  ...
  var paired=!!Sync.partnerId;
  if(!paired){el.innerHTML='';return}
  el.innerHTML='<div style="display:flex;gap:8px;padding:8px 0"><input id="msg-input" class="input" placeholder="说点什么…" style="flex:1;text-align:left;border-radius:24px" onkeydown="if(event.key===\'Enter\')Weather.sendMsg()"><button class="btn-primary" onclick="Weather.sendMsg()" style="border-radius:24px;padding:10px 16px">发送</button><button class="btn-secondary" onclick="Doodle.openSendMode()" style="border-radius:24px">🎨</button></div>';
},
